...
1 Can you write a solution that will return false for both [] and [ x ] (an empty array and
one with a single number) though? (This is an empty array and one with a single number and is not tested for,
