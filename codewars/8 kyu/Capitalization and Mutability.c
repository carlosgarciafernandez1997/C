...
{
  word[0] = toupper(word[0]); //  something goes wrong here !
  return word;
}
*/
#include <ctype.h>
...
char *capitalize_word (char *word)
{
  char *result=calloc(strlen(word),sizeof(char));
  memcpy(result, word, strlen(word) * sizeof(char));
  result[0] = toupper(word[0]); //  something goes wrong here !
  return result;
}